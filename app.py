...
from engine.bet_math import (
    ...
    decision_label,
    estimated_nfl_cover_probability,
    fair_line_probability,
    format_american,
    implied_probability,
    kelly_fraction,
    ...
    required_nfl_spread_edge,
    safe_int,
    stake_to_win,
    tennis_bet_confidence,
    tennis_confidence_meter,
    tennis_probability_confidence_band,
...
@st.cache_data(ttl=1800, show_spinner=False)
def fetch_api_tennis_today(target_date=None):
    """Load ATP/WTA fixtures from API-Tennis for the given date (defaults to today), without consuming Odds API credits."""
    ...
    query_date = target_date or datetime.now(EASTERN_TZ).date()
    try:
...
def fetch_api_tennis_odds_today(target_date=None):
    """Load API-Tennis pre-match moneyline markets for the given date (defaults to today), as a fallback source."""
    if not API_TENNIS_AVAILABLE or APITennisClient is None:
        return {}, {"source": "unavailable", "error": API_TENNIS_IMPORT_ERROR or "API-Tennis client unavailable"}
    query_date = target_date or datetime.now(EASTERN_TZ).date()
    try:
        client = APITennisClient()
        response = client.get_odds(query_date, query_date)
...
def normalize_api_tennis_schedule(fixtures, api_tennis_odds=None, target_date=None):
    """Normalize API-Tennis fixtures into the same basic columns as the automatic slate."""
    rows = []
    schedule_date = target_date or datetime.now(EASTERN_TZ).date()
...
def combine_tennis_slate(api_key, tennis_items, target_date=None):
    ...
    for item in tennis_items:
        ...
        try:
            ...
            frame = normalize_api_slate(api_events, event_title, target_date=target_date)
            if not frame.empty:
                frames.append(frame)
        except Exception as exc:
...
def normalize_api_slate(events, sport_title, target_date=None):
    rows = []
    schedule_date = target_date or datetime.now(EASTERN_TZ).date()
    for event in events:
        try:
            ...
            continue
        if start_et.date() != schedule_date:
            continue
...
def _breakeven_accuracy_summary(rows):
    """Average breakeven win% (vig-inclusive) needed on the actual prices bet, vs. actual accuracy."""
    graded = [row for row in rows if normalize_prediction_result(row) in ("Correct", "Incorrect")]
    breakevens = []
    for row in graded:
        odds = _analysis_market_line_numeric(row)
        if odds is not None:
            breakevens.append(implied_probability(odds))
    if not breakevens:
        return None
    avg_breakeven = sum(breakevens) / len(breakevens)
    actual_correct = sum(normalize_prediction_result(row) == "Correct" for row in graded)
    actual_accuracy = actual_correct / len(graded)
    return {
        "breakeven": avg_breakeven,
        "actual": actual_accuracy,
        "delta_pts": (actual_accuracy - avg_breakeven) * 100,
        "graded": len(graded),
    }
...
def _analyzed_pairs_for_date(target_date, rows):
    ...
    for row in rows or []:
        ...
        if str(row.get("event_date", ""))[:10] != date_str:
            continue
        a = str(row.get("participant_a", "")).strip().lower()
        b = str(row.get("participant_b", "")).strip().lower()
        if a and b:
...
def _run_and_log_daily_slate_tennis_analysis(
    tournament_name, player_a_name, player_b_name, odds_a_value, odds_b_value, event_date, slate_matches
):
    """Run the tennis model for a Daily Slate matchup and persist it to the real Analysis Log.

    Shared by the single "Analyze This Tennis Match" button and "Analyze All" so every
    quick-analyzed match actually lands in the Archive/Analysis Log and Browse-by-Day
    stats -- not just an in-session preview that disappears on refresh.
    """
    ...
    inferred_format = (
        "Best of 5" if inferred_category == "Grand Slam" and "WTA" not in tournament_name.upper() else "Best of 3"
    )

    result = analyze_tennis_match(
    ...
    probability_a = float(result["win_probability"])
    ...
    token = _analysis_event_token(
        "Tennis",
        {
            ...
            "market_odds_a": odds_a_value, "market_odds_b": odds_b_value,
        },
    )
    saved = _save_universal_analysis({
        ...
        "event_name": f"{player_a_name} vs {player_b_name}",
        "participant_a": player_a_name, "participant_b": player_b_name,
        ...
        "input_snapshot": {
            "player_a": player_a_name, "player_b": player_b_name,
            "tournament": tournament_name, "surface": inferred_surface,
            "tournament_category": inferred_category, "match_format": inferred_format,
            "market_odds_a": odds_a_value, "market_odds_b": odds_b_value,
        },
        "analysis_snapshot": {
    ...
    market_prob_a, market_prob_b, _ = no_vig_probabilities(odds_a_value, odds_b_value)
    return {
        "player_a": player_a_name,
        "player_b": player_b_name,
...
if active_top_page == "Daily Slate":
    ...
    st.caption(
        "Automatically load a market card without changing the existing manual slate or Analysis Engine. "
        "Pick a future date below to line up analysis ahead of time."
    )

    slate_today = datetime.now(EASTERN_TZ).date()
    slate_selected_date = st.date_input(
        "Slate date",
        value=slate_today,
        min_value=slate_today,
        max_value=slate_today + timedelta(days=6),
        key="daily_slate_target_date",
        help="Odds this far out can still move before match day -- treat early analysis as a preview, "
             "and re-run it closer to game time if the market line shifts.",
    )
    if slate_selected_date != slate_today:
        st.info(
            f"Showing the market card for {slate_selected_date.strftime('%A, %B %d')}. "
            "Not every book posts odds this far ahead, so some matches may still be missing until closer to the day."
        )
    ...
    if not api_key:
        ...
        try:
            ...
            if available_choices:
                ...
                with st.spinner(f"Loading the market slate for {slate_selected_date.strftime('%b %d')}..."):
                    tennis_load_errors = []
                    if available_choices[selected_label] == "__all_tennis__":
                        api_tennis_fixtures, api_tennis_status = fetch_api_tennis_today(slate_selected_date)
                        api_tennis_odds, api_tennis_odds_status = fetch_api_tennis_odds_today(slate_selected_date)
                        api_tennis_schedule = normalize_api_tennis_schedule(
                            api_tennis_fixtures, api_tennis_odds, target_date=slate_selected_date
                        )
                        if tennis_items:
                            odds_api_slate, usage, tennis_load_errors = combine_tennis_slate(
                                api_key, tennis_items, target_date=slate_selected_date
                            )
                        else:
                ...
                if available_choices[selected_label] == "__all_tennis__":
                    with st.expander("Tennis API diagnostics", expanded=automatic_slate.empty):
                        ...
                        st.write(f"API-Tennis fixtures scheduled for {slate_selected_date.strftime('%b %d')}: **{schedule_count}**")
                ...
                if automatic_slate.empty:
                    slate_date_label = slate_selected_date.strftime('%b %d')
                    if available_choices[selected_label] == "__all_tennis__":
                        if 'api_tennis_status' in locals() and api_tennis_status.get("error"):
                            st.info(
                                f"Macabets could not load the {slate_date_label} tennis schedule from API-Tennis or live prices from The Odds API. "
                                "Open diagnostics above for the provider errors."
                            )
                        else:
                            st.info(f"API-Tennis is not reporting any ATP/WTA singles fixtures scheduled for {slate_date_label} yet.")
                    else:
                        st.info(f"No {selected_label} events with US moneyline odds are scheduled for {slate_date_label} yet.")
                else:
                    if available_choices[selected_label] == "__all_tennis__":
                    ...
                    def _slate_event_label(idx):
                        ...
                        mark = "✓ " if pair_key in slate_analyzed_pairs else ""
                        return f"{mark}{row['time_et']} — {row['participant_a']} vs {row['participant_b']}"

                    selected_event_index = st.selectbox(
                        "Automatic event",
                        event_options,
                        format_func=_slate_event_label,
                    ...
                    if analyze_col.button(
                        ...
                        if pd.isna(selected_event["odds_a"]) or pd.isna(selected_event["odds_b"]):
                            ...
                            # Keep the Analysis Engine prefilled too, so if the user does open
                            # that tab later for the full breakdown, it already shows this
                            ...
                            # pick up later -- that queuing approach required the user to
                            # actually visit that tab before anything happened, and nothing
                            ...
                            with st.spinner("Macabets is analyzing the matchup..."):
                                try:
                                    ...
                                    if outcome["saved"]:
                                        st.toast(
                                            f"✅ Analysis complete & logged: {player_a_name} vs {player_b_name}. "
                                        ...
                                        st.toast(
                                            ...
                                            "(not saved to the Analysis Log -- check Supabase config).",
                                            icon="⚠️",
                                        )
                                except Exception as exc:
                        ...
                        if not to_run:
                            ...
                            for i, (idx, row) in enumerate(to_run, start=1):
                                try:
                                    ...
                                    batch_errors.append(f"{row['participant_a']} vs {row['participant_b']}: {exc}")
                                progress.progress(i / len(to_run), text=f"Analyzing {i} of {len(to_run)}...")
                            progress.empty()
                            _cached_universal_analysis_rows.clear()
                            st.session_state.daily_slate_batch_results = batch_results
                            if batch_results:
                                st.toast(f"✅ Analyzed and logged {len(batch_results)} match(es).", icon="✅")
                            if batch_errors:
                                st.warning(f"{len(batch_errors)} match(es) failed to analyze:")
                                for message in batch_errors:
                                    st.caption(message)

                    batch_results = st.session_state.get("daily_slate_batch_results")
                    if batch_results:
                        batch_df = pd.DataFrame([
                            {
                                "Matchup": f"{r['player_a']} vs {r['player_b']}",
                                "Model favors": r["projected_winner"],
                                "Win % (A)": r["model_win_probability_a"],
                                "Edge vs market (A)": r["edge_a"],
                                "Fair line (A)": r["fair_line_a"],
                                "Confidence": r["confidence"],
                                "Price": r["price_assessment"],
                                "Verdict": r["verdict"],
                            }
                        ...
                        sort_choice = st.selectbox(
                            "Sort last batch by",
                            ["Edge (biggest first)", "Confidence (highest first)", "Matchup"],
                            key="daily_slate_batch_sort",
                        )
                        if sort_choice == "Edge (biggest first)":
                            batch_df = batch_df.reindex(
                                batch_df["Edge vs market (A)"].abs().sort_values(ascending=False).index
                            )
                        elif sort_choice == "Confidence (highest first)":
                            batch_df = batch_df.sort_values("Confidence", ascending=False)
                        else:
                            batch_df = batch_df.sort_values("Matchup")
                        display_df = batch_df.copy()
                        display_df["Win % (A)"] = display_df["Win % (A)"].map(lambda v: f"{v:.1%}")
                        display_df["Edge vs market (A)"] = display_df["Edge vs market (A)"].map(lambda v: f"{v:+.1%}")
                        display_df["Fair line (A)"] = display_df["Fair line (A)"].map(lambda v: f"{v:+d}")
    ...
    if archive_page == "Analysis Log":
        ...
        if not analysis_db_configured():
            ...
            if available_days:
                ...
                day_breakeven = _breakeven_accuracy_summary(selected_day_rows)
                core_breakeven = _breakeven_accuracy_summary(day_core_rows)
                if day_breakeven or core_breakeven:
                    pieces = []
                    if day_breakeven:
                        pieces.append(
                            f"Day: {day_breakeven['actual']:.1%} actual vs {day_breakeven['breakeven']:.1%} "
                            f"breakeven ({day_breakeven['delta_pts']:+.1f} pts, {day_breakeven['graded']} graded)"
                        )
                    if core_breakeven:
                        pieces.append(
                            f"Core Zone: {core_breakeven['actual']:.1%} actual vs {core_breakeven['breakeven']:.1%} "
