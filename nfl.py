...
from engine.nfl_context import evaluate_nfl_context
...
class NFLAnalysis:
    ...
    base_fair_spread_home: float
    context_adjustment_home: float
    context_total_adjustment: float
    context_confidence_penalty: float
    context_game_stage: str
...
def analyze(
    *,
    ...
    context_result = evaluate_nfl_context(
        away_team=away_team,
        home_team=home_team,
        weather=weather,
        context=context,
    ...
    base_home_margin = home_power - away_power + applied_hfa
    base_fair_spread_home = round((-base_home_margin) * 2.0) / 2.0
    projected_home_margin = (
        base_home_margin + float(context_result["home_margin_adjustment"])
    )
    ...
    # The total remains market-anchored unless the user explicitly marks weather
    # as not yet reflected in the entered market number.
    fair_total = float(market_total) + float(context_result["total_adjustment"])
    ...
    confidence -= float(context_result["confidence_penalty"])
    ...
    result = NFLAnalysis(
        ...
        game_script=(
            ...
            f"coaching, schedule and special teams. The base team-quality line was {home_team} "
            f"{base_fair_spread_home:+.1f}; situational context moved the final line to "
            f"{home_team} {fair_spread_home:+.1f}. {underdog} remains live if it can keep the game "
            "out of the favorite's preferred script and win high-variance possessions."
        ),
        why_home_can_win=[
        ...
        foundation_notice=(
            f"NFL context-engine rating mode: {NFL_DATA_STATUS.get('rating_mode', 'starter priors')}. "
            "Performance categories are loaded from the saved nflverse snapshot when available; coaching and "
            "situational adjustments remain transparent manual inputs. The total stays market-anchored unless "
            "weather is explicitly marked as not yet priced into the entered number."
        ),
        away_power_rating=away_power,
        home_power_rating=home_power,
        home_field_points=applied_hfa,
        base_fair_spread_home=base_fair_spread_home,
        context_adjustment_home=float(context_result["home_margin_adjustment"]),
        context_total_adjustment=float(context_result["total_adjustment"]),
        context_confidence_penalty=float(context_result["confidence_penalty"]),
        context_game_stage=str(context_result["game_stage"]),
        context_breakdown=list(context_result["breakdown"]),
